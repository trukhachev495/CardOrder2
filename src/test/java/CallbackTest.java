...
public class CallbackTest {
    ...
    public void nameFieldValidation(){
        $("span[data-test-id='name'] input").setValue("Ivan trukchachev");
        $("span[data-test-id='phone'] input").setValue("+79508556982");
        $("[data-test-id='agreement']").click();
        $("[type=button]").click();
        $("span[data-test-id='name'].input_invalid .input__sub").shouldHave(exactText("Имя и Фамилия указаные неверно. Допустимы только русские буквы, пробелы и дефисы."));
    }
    ...
    public void phoneFieldValidation(){
        ...
        $("[data-test-id='agreement']").click();
        $("[type=button]").click();
        $("span[data-test-id='phone'].input_invalid .input__sub").shouldHave(exactText("Телефон указан неверно. Должно быть 11 цифр, например, +79012345678."));
    }
    ...
    public void notFillingInTheNameField(){
        ...
        $("[data-test-id='agreement']").click();
        $("[type=button]").click();
        $("span[data-test-id='name'].input_invalid .input__sub").shouldHave(exactText("Поле обязательно для заполнения"));
    }
    ...
    public void notFillingInThePhoneField(){
        ...
        $("[data-test-id='agreement']").click();
        $("[type=button]").click();
        $("span[data-test-id='phone'].input_invalid .input__sub").shouldHave(exactText("Поле обязательно для заполнения"));
    }
